...
def parse_tally_response(xml_text: str) -> dict:
    ...
    try:
        ...
        result["error_reason"] = f"Could not parse Tally response as XML: {exc}"
        return result
    ...
    if result["line_errors"]:
        ...
        result["error_reason"] = result["error_reason"] or (
            f"Tally EXCEPTIONS={result['exceptions']} with CREATED=0"
        )
    elif result["errors"] > 0:
        result["success"] = False
        result["error_reason"] = result["error_reason"] or f"Tally reported {result['errors']} error(s)"
    else:
        result["success"] = False
        result["error_reason"] = result["error_reason"] or "Voucher push returned CREATED=0"
